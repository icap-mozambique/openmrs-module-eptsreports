...
public class PrepReportDataset extends BaseDataSet {
  ...
  @Autowired private PrepReportCohortQueries prepReportCohortQueries;
  ...
  public CohortIndicatorDataSetDefinition getPrepReportDataSetDefinition() {
    ...
    final CohortDefinition cohortDefinition =
        this.prepReportCohortQueries.findPatientsOnPrepInjectable();

    final CohortIndicator indicator =
        this.eptsGeneralIndicator.getIndicator(
            "findPatientsOnPrepInjectable", EptsReportUtils.map(cohortDefinition, mappings));
    ...
    dataSetDefinition.addDimension(
        "homosexual",
        EptsReportUtils.map(this.prepReportCohortQueries.getPrepKp(PrepKp.HOMOSEXUAL), mappings));

    dataSetDefinition.addDimension(
        "drug-user",
        EptsReportUtils.map(this.prepReportCohortQueries.getPrepKp(PrepKp.DRUG_USER), mappings));

    dataSetDefinition.addDimension(
        "prisioner",
        EptsReportUtils.map(this.prepReportCohortQueries.getPrepKp(PrepKp.PRISIONER), mappings));

    dataSetDefinition.addDimension(
        "sex-worker",
        EptsReportUtils.map(this.prepReportCohortQueries.getPrepKp(PrepKp.SEX_WORKER), mappings));
    ...
    dataSetDefinition.addDimension(
        "couples-result",
        EptsReportUtils.map(
            this.prepReportCohortQueries.getPrepKp(PrepKp.COUPLES_RESULT_ARE_DIFFERENT), mappings));

    dataSetDefinition.addDimension(
        "adolescents-at-risk",
        EptsReportUtils.map(
            this.prepReportCohortQueries.getPrepKp(PrepKp.ADOLESCENTS_AND_YOUTH_AT_RISK),
            mappings));

    dataSetDefinition.addDimension(
        "pregnant",
        EptsReportUtils.map(
            this.prepReportCohortQueries.getPregnantAndBF(PrepKp.PREGNANT), mappings));

    dataSetDefinition.addDimension(
        "breastfeeding",
        EptsReportUtils.map(
            this.prepReportCohortQueries.getPregnantAndBF(PrepKp.BREASTFEEDING), mappings));
    ...
    dataSetDefinition.addColumn(
        "P-MILITARY", "Military", EptsReportUtils.map(indicator, mappings), "military=military");

    dataSetDefinition.addColumn(
        "P-MINER", "Miner", EptsReportUtils.map(indicator, mappings), "miner=miner");

    dataSetDefinition.addColumn(
        "P-DRIVER", "Driver", EptsReportUtils.map(indicator, mappings), "driver=driver");
    ...
    dataSetDefinition.addColumn(
        "P-PREGNANT", "Pregnant", EptsReportUtils.map(indicator, mappings), "pregnant=pregnant");
