...
@Component
public class TxCurrAPEs extends BaseDataSet {
  ...
  public CohortIndicatorDataSetDefinition constructTxCurrDataset(
      final boolean currentSpec, String type) {

    final CohortIndicatorDataSetDefinition dataSetDefinition =
        new CohortIndicatorDataSetDefinition();
    dataSetDefinition.setName("TX_CURR APEs Data Set");
    dataSetDefinition.addParameters(this.getParameters());

    final String mappings = "startDate=${startDate},endDate=${endDate},location=${location}";

    final CohortDefinition txCurrAPECompositionCohort =
        this.txCurrCohortQueries.findAPEsOnPatientsWhoAreActiveOnART();

    final CohortIndicator txCurrAPEIndicator =
        this.eptsGeneralIndicator.getIndicator(
            "findPatientsonAPEsWhoAreActiveOnART",
            EptsReportUtils.map(txCurrAPECompositionCohort, mappings));

    final CohortDefinition txCurrAPESViralLoadComposition =
        this.txCurrCohortQueries.findPatientWithViralLoadEventWithinPast12Months();

    final CohortIndicator txCurrAPESViralLoadIndicator =
        this.eptsGeneralIndicator.getIndicator(
            "APEsOnPatientsWhoAreActiveOnARTWithViralLoadEvent",
            EptsReportUtils.map(txCurrAPESViralLoadComposition, mappings));

    final CohortDefinition txCurrAPESViralLoadLessThanThousandComposition =
        this.txCurrCohortQueries.findPatientWithViralLoadLessThan1000WithinPast12Months();

    final CohortIndicator txCurrAPESViralLoadLessThanThousandIndicator =
        this.eptsGeneralIndicator.getIndicator(
            "APEsOnPatientWithViralLoadLessThan1000WithinPast12Months",
            EptsReportUtils.map(txCurrAPESViralLoadLessThanThousandComposition, mappings));
    ...
    if (type.equals("C")) {
      dataSetDefinition.addColumn(
          "APES",
          "APEs_AND_TX_CURR: APEs Currently on ART",
          EptsReportUtils.map(txCurrAPEIndicator, mappings),
          "");
    ...
    if (type.equals("V")) {
      dataSetDefinition.addColumn(
          "VIRALOAD",
          "APEs_AND_TX_CURR_VIRAL_LOAD_EVENT: APEs with Viral Load Event Currently on ART",
          EptsReportUtils.map(txCurrAPESViralLoadIndicator, mappings),
          "");

      this.addColums(
          dataSetDefinition,
          mappings,
          txCurrAPESViralLoadIndicator,
          UNDER_ONE,
          ONE_TO_FOUR,
          FIVE_TO_NINE,
          TEN_TO_FOURTEEN,
    ...
    if (type.equals("T")) {
      dataSetDefinition.addColumn(
          "THOUSAND",
          "APEs_AND_TX_CURR_VIRAL_LESS_THAN_THOUSAND: APEs with Viral Load less than 1000 and Currently on ART",
          EptsReportUtils.map(txCurrAPESViralLoadLessThanThousandIndicator, mappings),
          "");

      this.addColums(
          dataSetDefinition,
          mappings,
          txCurrAPESViralLoadLessThanThousandIndicator,
          UNDER_ONE,
          ONE_TO_FOUR,
          FIVE_TO_NINE,
          TEN_TO_FOURTEEN,
