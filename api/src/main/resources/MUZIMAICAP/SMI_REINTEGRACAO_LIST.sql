...
select 	LISTA1.*,
		...
		if(LISTA1.ordem in (1,11,12,13),1,0) tarv_elegivel,
		if(LISTA1.ordem in (2,3,4,5,6,7,8,9,10),'TARV','') estado_tarv,
		if(LISTA1.ordem in (1,2,3,4,5,6,7,8),TRUE,FALSE) gravida,
		...
		concat(ifnull(referencia_nome.referencia_nome,''),' ',ifnull(referencia_apelido.referencia_apelido,'')) pessoa_referencia,
		finalkptable.keypop,
...
(
	...
	(	
		select 	gravida_faltoso.patient_id,			
				'GRAV/LACT Faltosa' as TIPO_PACIENTE,
				1 as ordem
		...
		select 	consultaRecepcao.patient_id,			
			'GRAV/LACT Abandono' as TIPO_PACIENTE,
			4 as ordem
		...
		group by patient_id    
		having 	max(proximo_levantamento) between date_add(:endDate, interval -73 day) and date_add(:endDate, interval -67  day) or  
		...
		/*	select 	lactanteMaxFila.patient_id,
					'FALTOSA DEPOIS LACTANTE' as TIPO_PACIENTE,
					4 as ordem
			from 
			(
		...
		select 	coorte12meses_final.patient_id,
				'GRAV/LACT AMA' as TIPO_PACIENTE,
				6 as ordem
		from 
		(
		...
		where 	(data_estado is null or (data_estado is not null and  data_fila > data_estado)) and date_add(data_usar, interval 28 day) >=:endDate and ultimaCarga.value_numeric>50 and 
				(
					ultimaCarga.data_carga between date_add(:endDate, interval -13 day) and date_add(:endDate, interval -7  day) or 
					ultimaCarga.data_carga between date_add(:endDate, interval -28 day) and date_add(:endDate, interval -22  day) or 
					ultimaCarga.data_carga between date_add(:endDate, interval -70 day) and date_add(:endDate, interval -63  day) or 
					ultimaCarga.data_carga between date_add(:endDate, interval -91 day) and date_add(:endDate, interval -84  day)  
	...
	) gravida on gravida.patient_id=abandono.patient_id		
		inner join person p on p.person_id=abandono.patient_id	
	where p.gender='F'
...
left join 
(	select person_id,value_text referencia_nome
	from obs
	where voided=0 and concept_id=1441 and location_id=:location and obs_datetime<=:endDate
	group by person_id
) referencia_nome on referencia_nome.person_id=LISTA1.patient_id
left join 
(	select person_id,value_text referencia_apelido
	from obs
	where voided=0 and concept_id=1442 and location_id=:location and obs_datetime<=:endDate
	group by person_id
) referencia_apelido on referencia_apelido.person_id=LISTA1.patient_id
left join 
...
(
	...
	/*left join
	(
		select 	p.patient_id, max(obsState.obs_datetime) data_inicio_tpi  																		
		from 	patient p														 			  															
				inner join encounter e on p.patient_id=e.patient_id																				 		
		...
		where 	e.voided=0 and p.voided=0 and obsState.obs_datetime<=:endDate	 			  									
	...
	(
		...
		select p.patient_id, max(e.encounter_datetime) data_final_inh  																		
		 ...
		 where 	e.voided=0 and p.voided=0 and e.encounter_datetime<=:endDate	 			  									
				and o.voided=0 and o.concept_id=23985 and o.value_coded in (656,23982) and e.encounter_type=60 and  e.location_id=:location	  		 	
				and seguimentoTPT.voided =0 and seguimentoTPT.concept_id =23987 and seguimentoTPT.value_coded=1267 							
		 group by p.patient_id
...
(	
	...
	(
		...
		(
			...
			/* Removed by cella email 12.07.2022
...
) saida on saida.patient_id=lista.patient_id

where saida.patient_id is null

group by lista.patient_id
