-- Lista 2: SAAJ
select 	finalList.*,
		if(finalList.intervencao in ('DCP','AMA','CM') and tb is null,'AJM','') GRUPO_APOIO
from 
(
select lista.*,
...
(
	select 	LISTA1.patient_id,
			...
			LISTA1.ordem,			timestampdiff(MONTH,inicio.data_inicio,:endDate) mesesTARV,
...
(
	...
	select levMarcado.patient_id,			
		...
		(
			...
			group by patient_id    
			having 	max(proximo_levantamento) between date_add(:endDate, interval 46 day) and date_add(:endDate, interval 56 day)
		) levMarcado
		inner join 
...
inner join person per on per.person_id=LISTA1.patient_id
left join 
(	
...
(
	...
	(
		...
		group by p.patient_id
	) revelacao inner join person pe on pe.person_id=revelacao.patient_id
	where timestampdiff(year,birthdate,:endDate) between 10 and 14
) revelacao on LISTA1.patient_id=revelacao.patient_id
left join 
(
		...
		(
			...
			(
				...
				/* Removed by Cella on 12.07.2022
...
where ((timestampdiff(year,per.birthdate,:endDate) between 15 and 24) or revelacao.patient_id is not null) and gravidaLactante.patient_id is null and 
		(
			filhos.patient_id is null or filhos.nrFilhos=1
		)
