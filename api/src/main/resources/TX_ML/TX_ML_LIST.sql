...
select concat(ifnull(concat(' ',pad3.address5), ' '),ifnull(concat('  ',pad3.address1), '  '),ifnull(concat(', ',pad3.address3), '  ')) address3 ,
 pid.identifier identifier ,
 concat(ifnull(pn.given_name,''),' ',ifnull(pn.middle_name,''),' ',ifnull(pn.family_name,'')) name,
 p.gender gender, 
 floor(datediff(:endDate,birthdate)/365) age,
 pat.value patient_contact,
 confident.confident_contact confidant_contact, 
 confident_name.confident_name confidant_name,
 case (saidaFR.value_coded)
	...
	IF(data_abandono.expected_date is not null, DATEDIFF(:endDate, data_abandono.expected_date ), null)  dias_intererrupcao,
    txml.data_inicio,
    txml.data_entrada_obito data_obito
...
from (
 ...
 inner join person p on p.person_id=txml.patient_id
 left join   ( 
	  ...
	  ) pad3 on pad3.person_id=txml.patient_id 
	  left join( 
	  ...
	  ) pn on pn.person_id=txml.patient_id 
	  left join  ( 
	  ...
	  ) pid on pid.patient_id=txml.patient_id
	  left join person_attribute pat on pat.person_id=txml.patient_id and pat.person_attribute_type_id=9 and pat.value is not null and pat.value<>'' and pat.voided=0
                         	LEFT JOIN (
                         		SELECT max_confident.patient_id, max_confident.max_contat, confident_contact.confident_contact FROM (
                         			...
                         			INNER JOIN encounter e ON o.encounter_id = e.encounter_id
                         				WHERE o.voided = 0 AND e.voided = 0 AND o.concept_id IN(1611,6224) AND e.encounter_type = 53
                         				AND o.obs_datetime <= :endDate  AND e.location_id = :location
                         	...
                         	)confident ON confident.patient_id = txml.patient_id
                         	LEFT JOIN (
                         		SELECT max_confident.patient_id, max_confident.max_contat, confident_name.confident_name FROM (
                         			SELECT o.person_id patient_id, MAX(o.obs_datetime) max_contat FROM obs o
                         			INNER JOIN encounter e ON o.encounter_id = e.encounter_id
                         				WHERE o.voided = 0 AND e.voided = 0 AND o.concept_id = 1740 AND e.encounter_type = 53
                         				AND o.obs_datetime <= :endDate  AND e.location_id = :location
                         					GROUP BY o.person_id
                         		)max_confident 
                         		
                         		INNER JOIN (
                         			SELECT o.person_id patient_id, o.obs_datetime, o.value_text confident_name FROM obs o
                         				INNER JOIN encounter e ON o.encounter_id = e.encounter_id
                         					WHERE o.voided = 0 AND e.voided = 0 AND o.concept_id = 1740 AND e.encounter_type = 53
                         					AND o.obs_datetime <= :endDate  AND e.location_id = :location
                         						GROUP BY o.person_id
                         		)confident_name ON confident_name.patient_id = max_confident.patient_id AND confident_name.obs_datetime = max_confident.max_contat
                         		GROUP BY max_confident.patient_id
                         	
                         	)confident_name ON confident_name.patient_id = txml.patient_id
					...
					Select 	p.patient_id,max(o.obs_datetime) data_saida, o.value_coded 
									...
									where 	p.voided=0 and e.voided=0 and o.voided=0 and e.encounter_type = 53 and 
											o.concept_id = 6272 and o.value_coded in (1707, 1366, 1706, 1709) and
											o.obs_datetime between :startDate  and :endDate and e.location_id=:location
									group by p.patient_id

					) saidaFR on saidaFR.patient_id = txml.patient_id
			...
			LEFT JOIN (
					...
					)saidaPR on saidaPR.patient_id = txml.patient_id
			LEFT JOIN                                  
					(
            SELECT max_expected.patient_id, date_add(MAX(expected_date),interval 28 day) expected_date FROM (
                                     ...
                                     GROUP BY max_expected.patient_id
                                     ) data_abandono on data_abandono.patient_id = txml.patient_id
